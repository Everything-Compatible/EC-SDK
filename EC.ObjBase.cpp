...
{
    MessageBoxA(NULL, (std::string(Name) + " 不存在或不可默认构造！").c_str(), "EC SDK", MB_OK);
}
void ECImportFactoryHelper_Error_CopyConstruct(const char* Name)
{
    MessageBoxA(NULL, (std::string(Name) + " 不存在或不可复制构造！").c_str(), "EC SDK", MB_OK);
}
void ECImportFactoryHelper_Error_Destroy(const char* Name)
{
    MessageBoxA(NULL, (std::string(Name) + " 不存在析构接口！").c_str(), "EC SDK", MB_OK);
}
