...
public:
	...
	{
		return static_cast<SwizzleManagerClass&>(SwizzleManagerClass::Instance).Swizzle(指针);
	}

	// 原版游戏中的所有对象都会保存它的 this 指针的值到输出流中 , 然后通过调用这个函数来记录保存的 this 指针的值和新的指针的值
	HRESULT RegisterChange(void* 旧的指针, void* 新的指针)
	{
		return static_cast<SwizzleManagerClass&>(SwizzleManagerClass::Instance).Here_I_Am((long)旧的指针, 新的指针);
	}
